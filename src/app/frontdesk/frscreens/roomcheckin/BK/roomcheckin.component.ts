import { NgIf, NgFor, NgForOf, NgClass, CommonModule } from '@angular/common';
...
@Component({
  ...
  styleUrls: ['./roomcheckin.component.css']
})
...
export class RoomcheckinComponent implements OnInit {
  ...
  constructor(
    ...
    this.guest_form.get('booking_from')?.setValue('Web');
...
patchRoomForm(roomData: any, guestData: any) {
  ...
  updateGuest() {
        ...
        // localStorage.setItem('guest_id', this.selected_guest_info);
          // this.router.navigate(['/addreservation'], {
          //   queryParams: { guest_id: this.selected_guest_info }
          // });
    ...
    if (formValid) {
      ...
      const formattedRooms = this.rooms_info_form.value.rooms.map((room: any) => {
        ...
        return {
          ...
          loged_user:1,
          booking_id: this.selected_guest_info.booking_id ? this.selected_guest_info.booking_id : "-",
          room_id: room.room_no,
          guest_honor: this.selected_guest_info.guest_honor,
          guest_name: this.selected_guest_info.guest_name,
          guest_lastname:this.selected_guest_info.guest_lastname,
          guest_mobile: this.selected_guest_info.guest_mobile,
          companyname: this.selected_guest_info.companyname,
          guest_email: this.selected_guest_info.guest_email,
          guest_city: this.selected_guest_info.guest_city,
          address: this.selected_guest_info.address,
        ...
        next: (data) => {
          ...
          if (data.status) {
            ...
            this.roomBillingSummary = this.rooms.value.map((room: { plan: string; no_of_nights: string | number; room_no: any; }) => {
              ...
              const roomDetails = this.rooms_list.find((r: any) => r.roomid === this.room_id);
  ...
  addIdentification() {
    ...
    formData.append('guest_location_id', this.selected_guest_info.guest_details_location_id);
    formData.append('guest_room_id', this.rooms_info_form.value.rooms[0].room_no);
    formData.append('booking_id', this.selected_guest_info.booking_id);
  ...
  onSubmit() {
    const obj = {
      ...
      booking_id: this.selected_guest_info.booking_id,
      guest_room_id: 0,
      location_id: this.logininfo.location_id,
...
removePreview(index: number): void {
  ...
  createRoomGroup(): FormGroup {
    console.log(this.selected_guest_info)
    console.log(this.selected_guest_info.checkin_date)
    return this.fb.group({
      ...
      checkin_date: [this.selected_guest_info.checkin_date, Validators.required],
      no_of_nights: [this.selected_guest_info.no_of_nights, [Validators.required, Validators.min(1)]],
      checkout_date: [this.selected_guest_info.checkout_date, Validators.required],
      booking_status: ['confirmed', Validators.required]
    });
  }
