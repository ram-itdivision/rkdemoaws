...
export class AddreservationComponent implements OnInit {
      ...
      next: (rm_data) => {
        ...
        if (rm_data.status && Array.isArray(rm_data.content)) {
          ...
          this.route.queryParams.subscribe(params => {
            var bk_id = params['bk_id'];

            // // console.log(bk_id)
            if (bk_id) {
              this._reservationService.get_bookinginfo(bk_id).subscribe((res: any) => {
  ...
  // Auto-update when dueamount changes
  ...
  async setRoomsFromGuestInfo(guestList: any) {
    ...
    for (let i = 0; i < guestList.length; i++) {
      ...
      // // console.log(category +"6666666666666666")
      // Select plan
  ...
  get_rooms_plan_async(room_type_id: any, index: number): Promise<void> {
    return new Promise((resolve) => {
      ...
      // // console.log(category +"3333333333333333")
      forkJoin([plan$, room$]).subscribe({
        next: ([planRes, roomRes]) => {
          ...
          // // console.log(category +"444444444444444")
          resolve();
        },
        error: () => {
          this.room_plan_list[index] = [];
      ...
      // // console.log(category +"555555555555555")
    });
  }
  ...
  patchGuestForm(guestData: any) {
    this.guest_form.patchValue({
      ...
      billing_to: guestData.billing_to != '' ? guestData.billing_to : guestData.guest_name,
  ...
  addIdentification() {
    ...
    formData.append('guest_master_id', this.selected_guest_info[0].guest_bookin_id);
    formData.append('guest_location_id', this.selected_guest_info[0].guest_booking_location_id);
 ...
 getTotalRoomBilling(): number {
  ...
  getAvailableRooms(index: number) {
    // all rooms fetched for this row
